...
public class PhotCompWindow extends JFrame {
    ...
    // Match tolerance in degrees (default 2 arcsec = 0.000556 degrees)
    // This matches the tolerance used in DataConnector.findCrossMatches()
    private double matchToleranceDeg = 2.0 / 3600.0;
    ...
    public PhotCompWindow(DataConnector db) {
        ...
        rematchButton.addActionListener(e -> {
            try {
                // Parse using US locale to match the formatting (decimal point, not comma)
                String input = matchToleranceField.getText().trim();
                double arcsec = Double.parseDouble(input);
    ...
    private void performCrossMatching() {
        ...
        // Also check secondaryCatalogData (holds all secondary catalogs before merge)
        List<DataConnector.CatalogEntry> secondaryData = db.getSecondaryCatalogData();
        
        System.out.printf(java.util.Locale.US, "DEBUG PhotComp: Total merged stars=%d, Primary source=%d (%s), Unique sources in merged=%d, secondaryCatalogData size=%d\n",
            totalCount, primarySource, getSourceName(primarySource), allSources.size(), secondaryData.size());
        ...
        if (allSources.size() < 2 && secondaryData.size() == 0) {
            JOptionPane.showMessageDialog(this, 
                ...
                "Please load another catalog and try again.", 
        ...
        System.out.printf(java.util.Locale.US, "DEBUG PhotComp: Primary stars in merged=%d, Secondary stars in merged=%d\n",
            primaryCount, secondaryMergedCount);
        
        // CRITICAL: Always use secondaryCatalogData if available, because that's where
        // the un-merged secondary catalog entries are stored (before duplicate removal).
        // The merged arrays have already filtered out duplicates, so cross-matching
        // against them will miss most of the overlapping stars.
        boolean useSecondaryCatalogData = (secondaryData.size() > 0);
        
        if (useSecondaryCatalogData) {
            System.out.printf(java.util.Locale.US, "DEBUG PhotComp: Using secondaryCatalogData for cross-matching (tolerance=%.2f arcsec)\n",
                matchToleranceDeg * 3600.0);
        } else {
            System.out.printf(java.util.Locale.US, "DEBUG PhotComp: Using merged arrays for cross-matching (tolerance=%.2f arcsec)\n",
                matchToleranceDeg * 3600.0);
        }
        
        // Perform the actual cross-matching
        int primaryProcessed = 0;
        int primarySkippedBadMag = 0;
        ...
        System.out.printf(java.util.Locale.US, "DEBUG PhotComp: Processed %d primary stars (%d skipped bad mag), checked %d candidate pairs, found %d cross-matches\n",
            primaryProcessed, primarySkippedBadMag, candidatePairs, matchesFound);
        System.out.printf(java.util.Locale.US, "DEBUG PhotComp: Secondary sources found: %s\n", secondarySources);
